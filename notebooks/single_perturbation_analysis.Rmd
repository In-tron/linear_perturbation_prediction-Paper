...
strat_plot <- strat_data_expr_rank %>%
  ...
  ggplot(aes(x = rank, y = dist_mean)) +
    ...
    ggrepel::geom_text_repel(data = . %>% filter(dataset_name == "replogle_k562_essential") %>% 
                            ...
                            aes(label = annot, color = stage(method, after_scale = colorspace::darken(color, 0.5))), size = font_size_tiny / .pt,
...
plot_assemble(
  ...
  add_text("(B) Prediction error stratified by number of considered genes", 
...
forest_plot <- res_metrics_including_transfer |>
  ...
  ggplot(aes(x = l2$mean, y = method)) +
    ...
    geom_segment(aes(x = 1.1+1e-6, xend = Inf, color = dataset_name, 
                     alpha = I(case_when(includes_zero & l2$conf.higher > 1.1 ~ 0.3, ! includes_zero & l2$conf.higher > 1.1 ~ 1, .default = 0))), 
    ...
    labs(color = "", x = "Error relative to error of \\emph{Mean} (lower is better)") +
    coord_cartesian(xlim = c(0.85, 1.1), clip = "on") +
...
```{r}
plot_assemble(
  add_text("(A) Single perturbation prediction error", 
           x = 2.7, y = 1, fontsize = font_size, vjust = 1, fontface = "bold"),
...
```{r}
label_latex_scientific <- function(..., exponent_digits = 1){
  helper <- scales::label_scientific(...)
  function(x){
      res <- helper(x)
      spl <- stringr::str_split_fixed(res, "e", n = 2)
      mant <- spl[,1]
      exp <- spl[,2]
      exp <- sprintf(paste0("%0", exponent_digits, "i"), as.integer(exp))
      ifelse(exp == "", mant, paste0("$", mant, "\\times10^{", exp, "}$"))
  }
}


latex_10pow_labeller <- label_latex_scientific(digits = 1)
```
...
transfer_rpe1_pl <- transfer_rpe1 |>
  ggplot(aes(x = abs(rpe1_baseline - k562_baseline), y = abs(prediction - observed))) +
    ...
    labs(subtitle = sprintf("RPE1: slope= %.2f", coef(rpe1_lm)[2]),
         x = "Absolute difference of gene expression in RPE1 vs K562",
