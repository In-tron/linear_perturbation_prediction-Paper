...
strat_data_expr_rank <- full_long_df %>%
  filter(train != "train") %>%
  left_join(expr_rank_df %>% distinct(gene_name, dataset_name, seed, .keep_all=TRUE) %>%
...
strat_plot_dat <- strat_data_expr_rank %>%
  mutate(dataset_name = factor(dataset_name, levels = names(dataset_labels))) |>
...
writexl::write_xlsx(list(
  "Panel A" = main_pl_data2,
  "Panel B" = strat_plot_dat
), path = "../source_data/suppl-pearson_delta_performance_single_pert.xlsx")
```
...
writexl::write_xlsx(list(
  "Panel A" = main_pl_data2,
  "Panel C" = res_metrics_including_transfer
), path = "../source_data/single_perturbation_prediction.xlsx")
```
...
transfer_rpe1_pl <- transfer_rpe1 |>
  ggplot(aes(x = abs(rpe1_baseline - k562_baseline), y = abs(prediction - observed))) +
    ...
    labs(subtitle = sprintf("Trained on K562 to predict RPE1\nslope= %.2f", coef(rpe1_lm)[2]),
...
transfer_k562_pl <- transfer_k562 |>
  ggplot(aes(x = abs(rpe1_baseline - k562_baseline), y = abs(prediction - observed))) +
    ...
    scale_y_continuous(expand = expansion(add = 0), breaks = c(0, 1, 2, 3)) +
    scale_x_continuous(expand = expansion(add = 0), breaks = c(0, 1, 2, 3, 4)) +
    labs(subtitle = sprintf("Trained on RPE1 to predict K562\nslope= %.2f", coef(k562_lm)[2]),
         x = "Absolute difference of gene expression in RPE1 and K562",
         y = "Absolute difference of\nPredicted and Observed")
```
...
rpe1_target_cor_pl <- ggplot(rpe1_target_cor_dat, aes(x = abs(rpe1_baseline - k562_baseline), y = r2_delta)) +
    ...
    labs(subtitle = sprintf("Trained on K562 to predict RPE1\nslope = %.2f", coef(rpe1_target_lm)[2]),
...
k562_target_cor_pl <- ggplot(k562_target_cor_dat, aes(x = abs(rpe1_baseline - k562_baseline), y = r2_delta)) +
    ...
    scale_y_continuous(expand = expansion(add = 0), breaks = c(0, 0.5, 1)) +
    scale_x_continuous(expand = expansion(add = 0), breaks = c(0, 1, 2, 3)) +
    labs(subtitle = sprintf("Trained on RPE1 to predict K562\nslope = %.2f", coef(k562_target_lm)[2]),
         x = "Absolute difference of perturbation target\ngene expression in RPE1 and K562",
         y = "Pearson Delta")
```
...
plot_assemble(
  ...
  add_text("(B) Read-out gene error depends its on diff. expression between K562 and RPE1", 
  ...
  add_text("(C) Perturbation correlation depends on the target gene diff. expression between K562 and RPE1", 
...
```{r, paged.print=FALSE}
writexl::write_xlsx(list(
  "Panel A" = k562_rpe1_comp,
  "Panel C" = bind_rows(rpe1 = rpe1_target_cor_dat, k562 = k562_target_cor_dat, .id = "data")
), path = "../source_data/suppl-k562_rpe1_transfer.xlsx")
```
