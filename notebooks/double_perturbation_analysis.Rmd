...
```{r, paged.print=FALSE}
non_additive_colors <- c("Additive" = "lightgrey", "Non-additive" = "#00BA38", "Synergy" = "#fdc086", "Buffering" = "#beaed4", "Below baseline" = colorspace::darken("#f0027f", 0.5))

gene_response_label_df <- ground_truth_df |>
  ...
  filter(label != "Other non-additive") |>
  mutate(label = factor(label, levels = c("Additive", "Buffering", "Synergy", "Below baseline"))) 
...
tp_fdp_data_pre <- inter_pred_dat %>%
...
```{r, paged.print=FALSE}
prediction_label_vs_true_df <- tp_fdp_data_pre |>
  ungroup() |>
  # filter(fdp < 0.5) |>
  slice_head(n = 500, by = c(method, seed)) |>
  ...
  count(method, interaction_label, prediction_label, seed) |>
...
label_text <- str_trim("
363 of the 500 non-additive interactions had 
consistent single perturbation direction. 
231 of those were predicted to be buffering, 
but only 58\\% of actually were.
")
...
mosaic_plot <- prediction_label_vs_true_df |>
  ...
  mutate(interaction_label = fct_rev(interaction_label)) |>
  ggplot(aes(x = frac, y = prediction_label)) + 
    ...
    scale_y_discrete(labels = c(buf = "Buffering prediction", syn = "Synergy prediction", anti = "Below baseline\nprediction"), expand = expansion(add = c(0.4, 0.2))) +
    scale_fill_manual(values = non_additive_colors) +
...
```{r, paged.print=FALSE}
stop("Make sure that I am applying the same filter regarding pert_same_dir as for the main figure")
...
plot_assemble(
  ...
  add_plot(pert_pred_comparison + guides(color = "none"), x = 85, y = 68, width = 95, height = 62),
  add_plot(my_get_legend(pert_pred_comparison + theme(legend.position = "right")), x = 163, y = 96, width = 17, height = 23),
