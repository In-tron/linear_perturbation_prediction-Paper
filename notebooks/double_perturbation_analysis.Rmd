...
```{r, paged.print=FALSE}
method_labels <- c("gears" = "GEARS", "scgpt" = "scGPT", "scfoundation" = "scFoundation",
                   "uce" = "UCE*", "geneformer" = "Geneformer*", "cpa" = "CPA",
                   "no_change" = "No Change", "additive_model" = "Additive")
...
approach_annot <- tibble(method = names(method_labels)) |>
  mutate(approach = case_when(
    ...
    method %in% c("scgpt", "scfoundation", "uce", "uce33", "geneformer") ~ "foundation_model",
...
obs_pred_corr_pl <- contr_res %>%
  ...
  ggplot(aes(x = obs_minus_baseline, y = pred_minus_baseline)) +
    ...
    annotate("rect", xmin = -0.95, ymin = 0.5, xmax = 0.1, ymax = Inf, fill = "white", alpha = 0.8) +
    ...
    ggh4x::facet_nested_wrap(vars(approach, method), 
                             nrow  =1, # ncol = 2, 
    ...
    labs(x = "observed expression minus control", y = "predicted expression\nminus control")
...
#   add_plot(main_pl_double_l2, x = 0, y = 4, width = 130, height = 47.5),
# 
#   add_text("(B)\\;\\;{\\textcolor{baseROrange}\\faCircle} CEBPE+CEBPB", x = 130, y = 1, fontsize = font_size, vjust = 1, fontface = "bold"),
#   add_plot(obs_pred_corr_pl, x = 130, y = 4, width = 40, height = 100),
#   
#   
...
inter_pred_dat <- res %>%
  ...
  pivot_longer(c(scgpt, gears, scfoundation, geneformer, uce, cpa, additive_model, no_change), names_to = "method") %>%
...
pert_pred_comparison <- inter_pred_dat %>%
  ...
  ggplot(aes(x = obs_minus_add, y = pred_minus_add)) +
    ...
    ggh4x::facet_nested_wrap(vars(approach, method), ncol = 6, axes = "x",
                             labeller = labeller(approach = as_labeller(approach_labels), method = as_labeller(method_labels)),
                             nest_line = TRUE, strip = ggh4x::strip_nested(clip = "off")) +
    coord_fixed(xlim = c(-1, 1), ylim = c(-1.2, 1.2)) +
    labs(x = "Observed expression minus additive model value", 
       y = "Predicted expression minus additive model value", 
       color = "") +
...
tp_fdp_pl <- tp_fdp_data %>%
  ...
  ggplot(aes(x = fdp, y = tpr)) +
    ...
    guides(color = legendry::guide_legend_group(key = legendry::key_group_split(sep = "\\|", reverse = TRUE), nrow = 2, direction = "horizontal")) +
...
plot_assemble(
  ...
  add_text("(B)\\;\\;{\\textcolor{baseROrange}\\faCircle} CEBPE+CEBPB", x = 2.7, y = 53, fontsize = font_size, vjust = 1, fontface = "bold"),
  add_plot(obs_pred_corr_pl, x = 0, y = 54, width = 129, height = 30),
  
  add_text("(C) Accuracy of non-additive predictions", x = 2.7, y = 85, fontsize = font_size, vjust = 1, fontface = "bold"),
  add_plot(tp_fdp_pl + theme(legend.position = "none"), x = 0, y = 87, width = 60, height = 50),
  add_plot(my_get_legend(tp_fdp_pl), x = 10, y = 137, width = 60, height = 10),
  add_text("(D) Prediction of non-additive perturbation effects", x = 63, y = 85, fontsize = font_size, vjust = 1, fontface = "bold"),
  add_plot(pert_pred_comparison + theme(legend.position = "none"), x = 63, y = 87, width = 90, height = 55),
  add_plot(my_get_legend(pert_pred_comparison), x = 100, y = 120, width = 60, height = 10),
  
  width = 183, height = 170, units = "mm", show_grid_lines = FALSE,
  latex_support = TRUE, filename = "../plots/perturbation_prediction.pdf"
)
```
