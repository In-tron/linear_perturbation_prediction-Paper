...
main_pl_double_l2 <- main_pl_data %>%
  ggplot(aes(x = label, y = l2)) +
    ...
    ungeviz::geom_hpline(data = . %>% summarize(l2 = mean(l2), .by = c(approach, label)), aes(y = l2), 
                         color = "red", width = 0.6, linewidth = 0.6) +
    ggbezier::geom_bezier(data = tibble(approach = "baseline", x = c(1, 1.5), y = c(8.5, 10)),
                          aes(x = x, y = y, angle = c(0, 90)), arrow = grid::arrow(type = "closed", ends = "first", length = unit(1, "mm"))) +
    geom_text(data = tibble(x = 1.5, y = 10.1, label = "CEBPE+CEBPB", approach = "baseline"), aes(x=x, y=y, label=label), hjust = 0.2, vjust = 0,
...
plot_assemble(
  ...
  add_text("(B) Prediction error stratified by the number of considered genes",
...
```{r, paged.print=FALSE}
non_additive_colors <- c("Additive" = "lightgrey", "Other non-additive" = "#767676", "Non-additive" = "#00BA38", "Synergy" = "#fdc086", "Buffering" = "#beaed4", "Below baseline" = colorspace::darken("#f0027f", 0.5))
...
gene_response_label_df_intermed <- ground_truth_df |>
  ...
  mutate(label = case_when(
    ...
    pert_same_dir & ctrl < (A+B-ctrl) & AB - (A + B - ctrl) < lower_thres & AB < ctrl ~ "Anti-Synergy",
    pert_same_dir & ctrl > (A+B-ctrl) & AB - (A + B - ctrl) < lower_thres & AB > ctrl ~ "Anti-Synergy",
...
# print(sprintf("Total (%i) and fraction (%.3f) of all interactions are 'Other non-additive' interactions", 
#               sum(gene_response_label_df_intermed$label == "Other non-additive"),
#               mean(gene_response_label_df_intermed$label == "Other non-additive")))

gene_response_label_df <- gene_response_label_df_intermed |>
  # tidylog::filter(label != "Other non-additive") |>
  mutate(label = factor(label, levels = c("Additive", "Other non-additive", "Buffering", "Synergy", "Below baseline")))
```
...
pert_pred_comparison <- pert_pred_comparison_df %>%
  ...
  ggplot(aes(x = obs_minus_add, y = pred_minus_add)) +
    ...
    scale_color_manual(values = non_additive_colors, 
                       labels = c("Additive", "Other non-additive" = "Other non-add.", "Buffering", "Synergy", "Below baseline"), drop = TRUE) +
...
mosaic_plot <- prediction_label_vs_true_df |>
  ...
  ggplot(aes(x = frac, y = prediction_label)) + 
    ...
    scale_y_discrete(labels = c(buf = "Buffering prediction", syn = "Synergy prediction", anti = "Below baseline prediction"), expand = expansion(add = c(0.4, 0.2))) +
    scale_fill_manual(values = non_additive_colors) +
...
cartoon_plot <-bar_df |>
  ...
  ggplot(aes(x = bar, y = height)) +
    ...
    annotate("text", x = "obs", y = 0.15, label = "Below\nbaseline", size = font_size_tiny / .pt, color = "black") +
...
plot_assemble(
  ...
  add_text("(B) Example: CEBPE+CEBPB {\\scriptsize\\textcolor{baseROrange}\\faCircle}", x = 128, y = 1, fontsize = font_size, vjust = 1, fontface = "bold"),
  ...
  add_text("(D) Composition of\n\\;\\;interaction effects", x = 43, y = 66, fontsize = font_size, vjust = 1, fontface = "bold"),
...
```{r, paged.print=FALSE}
good_synergy_preds <- tp_fdp_prec_recall_data_pre |>
  filter(seed == 1) |>
  slice_head(n = 500, by = c(method)) |>
  mutate(prediction_label = case_when(
    baseline < ref & ref > value & value >= baseline ~ "buf",
  ...
  filter(prediction_label == "buf" & interaction_label == "Synergy")
...
plot_data <- inter_pred_dat %>%
  ...
  inner_join(good_synergy_preds |> dplyr::select(method, gene_name, sel_pert = perturbation))

plot_data |>
  mutate(perturbation_int = as.integer(fct_reorder(perturbation, ref)), .by = c(gene_name, method)) |>
  mutate(true_nonadditive = obs_minus_add > upper_thres | obs_minus_add < lower_thres) %>%
  ggplot(aes(x = perturbation_int)) +
    geom_hline(aes(yintercept = baseline), alpha = 0.1) +
    ungeviz::geom_hpline(aes(y = ref), color = "darkgrey", linewidth = 0.3, width = 0.8) +
    geom_rect(aes(xmin = stage(perturbation_int, after_scale = x - 0.5), xmax = stage(perturbation_int, after_scale = x + 0.5), ymin = ref - lower_thres, ymax = ref + lower_thres),
              fill = "#e2e2e2", alpha = 0.3) +
    geom_rect(data = . %>% filter(perturbation == sel_pert), aes(xmin = stage(perturbation_int, after_scale = x - 0.5), xmax = stage(perturbation_int, after_scale = x + 0.5), ymin = -Inf, ymax = Inf),
    ...
    scale_color_manual(values = non_additive_colors, drop = FALSE) +
    scale_alpha_manual(values = c("TRUE" = 1, "FALSE" = 0.2)) +
    facet_wrap(vars(method, gene_name), ncol = 6) +
...
expression_variation_pl <- all_predictions |>
  summarize(.by = c(gene_name, method, seed, train),
            sd = sd(prediction)) |>
  ...
  ggplot(aes(x = sd)) +
    ...
    scale_x_continuous(transform = scales::pseudo_log_trans(sigma = 0.01), limits = c(-0.002, 1.6), 
                       breaks = c(0, 0.1, 0.5, 1.5), expand = expansion(mult = c(0, 0.1))) +
    scale_y_continuous(expand = expansion(mult = c(0, 0.1)), limits = c(0, 16000)) +
    facet_wrap(vars(method), labeller = as_labeller(c(ground_truth = "Ground Truth", method_labels)), scales = "free", nrow = 2) +
    labs(x = "pseudo log of the expression Standard Deviation per Gene")

expression_variation_pl
```

```{r}
plot_assemble(
  add_text("(A) The predictions of most methods vary less than the observed expression values", x = 2.7, y = 1, fontsize = font_size, vjust = 1, fontface = "bold"),
  add_plot(expression_variation_pl, x = 0, y = 4, width = 180, height = 80),


  width = 180, height = 90, units = "mm", show_grid_lines = FALSE,
  latex_support = TRUE, filename = "../plots/suppl-prediction_variation.pdf"
)
```


# Angle between perturbations AB, A, and B
...
#   inner_join(expr_rank_df %>% dplyr::select(gene_name, seed, expr_rank) %>% filter(expr_rank <= 1000), by = c("gene_name", "seed")) %>%
#   mutate(add =  (A + B - ctrl)) |>
#   mutate(AB = AB - ctrl, A = A - ctrl, B = B - ctrl) |>
#   summarize(angle_AB_A = acos(sum(AB * A) / (norm2(AB) * norm2(A))) / pi * 180,
#             angle_AB_B = acos(sum(AB * B) / (norm2(AB) * norm2(B))) / pi * 180,
#             angle_A_B = acos(sum(A * B) / (norm2(A) * norm2(B))) / pi * 180,
#             .by = c(pert_group)) |>
#   mutate(highlight = case_when(
#     rank(desc(angle_AB_B)) <= 2 ~ TRUE,
#     rank(desc(angle_AB_A)) <= 4 ~ TRUE,
#     rank(desc(angle_AB_A^0.2 + angle_AB_B^0.2)) <= 2 ~ TRUE,
#     rank(angle_AB_A + angle_AB_B) <= 3 ~ TRUE,
#     .default = FALSE
#   )) |>
#   ggplot(aes(x = angle_AB_A, y = angle_AB_B)) +
#     geom_hline(yintercept = 90) +
#     geom_vline(xintercept = 90) +
#     geom_abline() +
#     geom_point(aes(color = angle_A_B)) +
#     # ggrepel::geom_text_repel(data = . %>% filter(angle_AB_B > 70 | angle_AB_A > 70), aes(label = pert_group), size = 2) +
#     ggrepel::geom_text_repel(data = . %>% filter(highlight), aes(label = pert_group), size = 2) +
#     scale_x_continuous(limits = c(0, 120)) +
#     scale_y_continuous(limits = c(0, 120)) +
#     # scale_color_viridis_c(limits = c(0, 120), oob = scales::squish) +
#     scale_color_gradient2(midpoint = 90, limits = c(0, 180), expand = expansion(add = 0),
#                           breaks = c(0, 45, 90, 135, 180),
#                           mid = "#303030", high = colorspace::lighten("red", amount = 0.4), , low = colorspace::lighten("blue", amount = 0.4)) +
#     coord_fixed() +
#     labs(x = "angle(AB, A)", y = "angle(AB, B)", color = "angle(A, B)",
#          title = "Angle between expression in double perturbation minus control (AB) vs.\n expression in individual perturbations minus control (A,B)",
#          subtitle = "Top 1000 genes. Pattern is relatively stable across n.")
```
...
# ground_truth_df %>%
#   filter(seed == 1) |>
#   # filter(gene_name == "HBG1") |>
#   filter(str_detect(pert_group, "MAPK1")) |>
#   mutate(template = cbind(B, A)[,str_detect(pert_group, "MAPK1\\+")[1] + 1],
#          query = cbind(A, B)[,str_detect(pert_group, "MAPK1\\+")[1] + 1], .by = c(pert_group)) |>
#   ggplot(aes(x = template - ctrl, y = AB - ctrl)) +
#     # geom_hline(aes(yintercept = query)) +
#     geom_abline() +
...
```{r}
plot_assemble(
  add_text("(A) Analysis of the predicted and observed expression patterns for HBG2 and HBZ", x = 2.7, y = 1, fontsize = font_size, vjust = 1, fontface = "bold"),
